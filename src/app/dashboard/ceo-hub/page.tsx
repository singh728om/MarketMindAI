      ...
      {!analysis ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={item.id} className="p-4 rounded-2xl bg-slate-800/50 border border-white/5 flex items-center justify-between group hover:border-primary/30 transition-all">
                      <div className="flex items-center gap-3">
                        <item.icon size={16} className="text-primary" />
                        <span className="text-xs font-bold text-slate-200">{item.label}</span>
                      </div>
                      <input type="file" className="hidden" id={`file-${item.id}`} onChange={handleFileChange} />
                      <Button size="sm" variant="ghost" className="h-8 w-8 p-0 rounded-lg hover:bg-primary/10" asChild>
                        <label htmlFor={`file-${item.id}`} className="cursor-pointer">
                          <FileUp size={14} className="text-slate-500 group-hover:text-primary" />
                        </label>
                      </Button>
                    </div>
                  ))}
              ...
              {uploadedFiles.length > 0 && (
                <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
                  <p className="text-[10px] font-bold text-emerald-500 uppercase mb-2">Ingested Assets ({uploadedFiles.length})</p>
                  <div className="flex flex-wrap gap-2">
                    {uploadedFiles.map((f, i) => <Badge key={i} className="bg-slate-900 text-[8px] border-white/5">{f}</Badge>)}
                  </div>
                </div>
              )}
            </div>
          ...
          <Card className="rounded-[2.5rem] border-white/5 bg-slate-950 p-10 flex flex-col items-center justify-center text-center space-y-6 relative overflow-hidden">
            ...
            <div className="w-24 h-24 rounded-3xl bg-primary/10 flex items-center justify-center text-primary shadow-inner relative z-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            <Card className="rounded-3xl border-white/5 bg-slate-900/50 overflow-hidden">
              <CardHeader className="p-8 pb-4">
                ...
                <CardTitle className="text-2xl md:text-3xl font-headline text-white">Intelligence Feed</CardTitle>
              </CardHeader>
            ...
            <Card className="rounded-3xl border-white/5 bg-slate-900/50 overflow-hidden">
              <CardHeader className="border-b border-white/5 p-8 bg-white/5">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="bg-rose-500/10 text-rose-500 border-rose-500/20 px-3 py-1 font-bold">CRITICAL</Badge>
                </div>
              </CardHeader>
                  ...
                  {(analysis.leakageInsights || []).map((leak: any, i: number) => (
                    <div key={i} className="p-8 flex items-center justify-between gap-6 hover:bg-white/5 transition-colors group">
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-xl font-headline font-bold text-rose-500">₹{((analysis.metrics?.loss || 0) / 100000).toFixed(1)}L</p>
                      </div>
                    </div>
                  ))}
                </div>
